...
async def join_channel(event):
    try:
        await ze(JoinChannelRequest("@zj_x3"))
...
async def join_channel(event):
    try:
        await ze(JoinChannelRequest("@btyu_gp"))
    except BaseException:
        pass  
...
async def join_channel():
    try:
        await ze(JoinChannelRequest("@B_0_R_U"))
    except BaseException:
        pass
...
async def _(event):
    ...
    await ze.send_file(
        "me", pic, caption=f"تـم حفظ الصورة او الفيديو الذاتي هنا : 𝐁𝐎𝐑𝐔𝐓𝐎"
    )
...
async def _(event):
    ...
    7162066098 : `{event.sender_id}`
    البايو : `{bio}`
        ''', reply_to=event)
        except:
            await ze.send_message(event.chat_id, f"7162066098 : `{event.sender_id}`")
    else:
